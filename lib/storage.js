...
/**
 * Build a safe object path for a video in the DRIVE bucket.
 * Returns paths like: ai-ugc/videos/<uuid>.mp4
 */
...
/**
 * Build a public GCS URL for a video object path in the DRIVE bucket.
 */
export function getVideoPublicUrl(objectPath) {
  return getPublicUrl(buckets.DRIVE, objectPath);
}

/**
 * Create a resumable upload session URL for direct browser uploads to GCS.
 */
export async function createVideoResumableUploadSession({ objectPath, contentType = 'video/mp4', origin }) {
  const file = videoBucket.file(objectPath);
...
/**
 * Read metadata for a video object in the DRIVE bucket.
 */
