...
export async function createPipelineBatch({ name, pipeline, totalJobs, isMaster, masterConfig }) {
  const result = await sql`
    INSERT INTO pipeline_batches (name, pipeline, total_jobs, status, is_master, master_config)
    VALUES (${name}, ${JSON.stringify(pipeline)}, ${totalJobs || 0}, 'pending', ${isMaster || false}, ${masterConfig ? JSON.stringify(masterConfig) : null})
    RETURNING *
  `;
...
// Template Jobs CRUD
export async function createTemplateJob({ name, pipeline, videoSource, tiktokUrl, videoUrl, pipelineBatchId, modelId, regeneratedFrom = null }) {
  const totalSteps = pipeline.filter(s => s.enabled).length;
  const result = await sql`
    INSERT INTO template_jobs (name, pipeline, total_steps, video_source, tiktok_url, video_url, pipeline_batch_id, model_id, regenerated_from)
    VALUES (${name}, ${JSON.stringify(pipeline)}, ${totalSteps}, ${videoSource || 'tiktok'}, ${tiktokUrl || null}, ${videoUrl || null}, ${pipelineBatchId || null}, ${modelId || null}, ${regeneratedFrom || null})
    RETURNING *
  `;
