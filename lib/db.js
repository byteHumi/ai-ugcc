...
// Jobs CRUD
export async function createJob({ tiktokUrl, imageUrl, customPrompt, maxSeconds, batchId }) {
  const result = await sql`
    INSERT INTO jobs (tiktok_url, image_url, custom_prompt, max_seconds, batch_id)
    VALUES (${tiktokUrl}, ${imageUrl}, ${customPrompt || null}, ${maxSeconds || 10}, ${batchId || null})
    RETURNING *
  `;
...
// Posts CRUD
export async function createPost({ jobId, accountId, caption, videoUrl, platform, status, scheduledFor }) {
  const result = await sql`
    INSERT INTO posts (job_id, account_id, caption, video_url, platform, status, scheduled_for)
    VALUES (${jobId || null}, ${accountId || null}, ${caption || null}, ${videoUrl}, ${platform || 'tiktok'}, ${status || 'draft'}, ${scheduledFor || null})
    RETURNING *
  `;
...
export async function updatePost(id, updates) {
  const { status, publishedAt, externalPostId, error } = updates;
