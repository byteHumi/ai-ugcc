...
export async function getGeneratedImagesPage(limit, offset, options = {}) {
  ...
  const rows = await getGeneratedImagesRows(limit, offset, { modelId, createdAfter, sort });
  if (!includeTotal) {
    return { images: rows.map(transformGeneratedImage), total: null };
  }
  const countResult = await getGeneratedImagesCountRows({ modelId, createdAfter });
